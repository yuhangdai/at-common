...
import com.aotain.nms.common.constant.GlobalParams;
import com.aotain.nms.common.model.push.CommandPushDetail;
import com.aotain.nms.common.model.push.EmailPushMessageRecord;
import com.aotain.nms.common.model.push.PushInterfaceMessage;
import com.aotain.nms.common.model.push.PushMessage;
import com.aotain.nms.common.model.push.PushMessageRecord;
import com.aotain.nms.common.model.push.PushReceiver;
import com.aotain.nms.common.model.push.PushResponse;
import com.aotain.nms.common.model.push.SMSPushMessageRecord;
import com.aotain.nms.common.model.push.SendData;
import com.aotain.nms.common.model.push.WeChatSendData;
import com.aotain.nms.common.model.push.WechatPushMessageRecord;
import com.aotain.nms.common.model.smmscmd.Blacklist;
import com.aotain.nms.common.model.smmscmd.CodeList;
import com.aotain.nms.common.model.smmscmd.Command;
import com.aotain.nms.common.model.smmscmd.Command.Rule;
import com.aotain.nms.common.model.smmscmd.CommandRecord;
import com.aotain.nms.common.model.smmscmd.IdcInfoManage;
import com.aotain.nms.common.model.smmscmd.IdcInfoManage.CommandInfo;
import com.aotain.nms.common.model.smmscmd.LogQuery;
import com.aotain.nms.common.model.smmscmd.LogQuery.CommandInfo.DestIp;
import com.aotain.nms.common.model.smmscmd.NoFilter;
import com.aotain.nms.common.model.smmscmd.QueryView;
import com.aotain.nms.common.model.smmscmd.ReturnInfo;
import com.aotain.nms.common.tools.HttpUtils;
import com.aotain.nms.common.utils.push.PushSecurityTool.PushEncryptResult;
import com.aotain.nms.common.utils.push.PushSecurityTool.Tools;
import com.aotain.nms.common.utils.push.dao.PushMapper;
...
public class PushClient {
	...
	@Autowired
	private BaseRedisService<String, String, String> redisService;
	 ...
	 */
	public void pushCommand(int type, Long alarmId, String alarmHouse, String cmdJsonStr) {
		if (alarmHouse == null) {
			logger.info("告警机房信息为空：" + alarmHouse);
			return;
		}
		if (cmdJsonStr == null) {
			logger.info("告警详细信息为空：" + cmdJsonStr);
			return;
		}
		String value = pushMapper.getPushServiceModule();
		if ("3".equals(value)) {
			if (type != GlobalParams.COMMAND_INFO_MANAGE_TYPE) {
				return;
			}
		...
		List<String> alarmHouses = Arrays.asList(alarmHouse.split(","));
		CommandPushDetail msg = parseMessage(type, alarmHouses, cmdJsonStr);
		...
		sendData.setPushId(alarmId);
		sendData.setSubjectType(1L);
		// 调用推送方法
	 ...
	 */
	public void pushMessage(SendData message) {
		// 1. 准备推送参数
		String pushService = pushMapper.getPushService();
		String flags = "";
		if (pushService != null) {
			flags = pushService;
		} else {
			logger.error("告警推送方式为空");
			return;
		}
		...
		for (Integer pType : types) {
			...
			String subjectType = msgObj.getSubjectType() + "";
			String pushType = msgObj.getPushType() + "";
			String pushId = msgObj.getPushId() + "";
			String value = redisService.getHash("nms_push_" + subjectType, pushType + "_" + pushId);
			if (value == null) {
					...
					try {
						...
						if (res == null) {
							...
							if (res.getResultCode() == 0) {
								pushMapper.updatePushMessageStatus(msgObj.getPushId(), 2);
								logger.info("推送请求已发送：" + message);
								//添加推送成功的redis记录
								redisService.putHash("nms_push_" + subjectType, pushType + "_" + pushId, "1");
							} else {
								pushMapper.updatePushMessageStatus(msgObj.getPushId(), 3);
								logger.error("推送请求失败,resultCode:" + res.getResultCode() + ",msg:" + res.getMsg() + ",data=" + message);
							}
						}
					} catch (IOException e) {
						pushMapper.updatePushMessageStatus(msgObj.getPushId(), 3);
				...
				logger.info("告警信息已经推送过，不再重复推送，subjectType:" + subjectType + ",pushType:" + pushType + ",pushId:" + pushId);
			}
		}
	}
	 ...
	 * @param sendData
	 */
	public void pushMessage(List<SendData> sendData){
		if (sendData==null||sendData.isEmpty()){
			return;
		}
		for (int i=0;i<sendData.size();i++){
			pushMessage(sendData.get(i));
		}
	}
	...
	private PushMessage format(SendData msg, Integer pushType) {
		...
		tpl.setPushId(msg.getPushId());
		tpl.setPushType(pushType);
		tpl.setSubjectType(msg.getSubjectType());
		tpl.setStatus(0); // 提交
		// 组装发送内容
		...
		tpl.setEmailReceiver(msg.getEmailReceiver());
		tpl.setPhoneReceiver(msg.getPhoneReceiver());
		return tpl;
	}
	...
	private PushMessageRecord buildRecord(PushMessage msgObj) {
		...
		case 1: // Email
			...
			receiver.setEmailReceiver(msgObj.getEmailReceiver());
		...
		case 2: // 短信
			...
			receiver1.setPhoneReceiver(msgObj.getPhoneReceiver());
	...
	private PushResponse pushMessage(PushInterfaceMessage pushMsg) throws IOException {
		...
		formparams.add(v9);
		formparams.add(v10);
		String rs = HttpUtils.postRequest(PUSH_URL, formparams, "UTF-8");
		try {
	 ...
	 * @return
	 */
	private CommandPushDetail parseMessage(int type, List<String> alarmHouses, String cmdJsonStr) {
		...
		if (GlobalParams.COMMAND_BASIC_DATA_QUWERY_TYPE == type) {
			...
			content = JSON.toJSONString(userIds);//parseUsers(userIds);
		}
		// 2. 信息安全管理指令
		...
		msg.setHouseStrList(alarmHouses);
...
//	}
	...
	private String paserQueryCmd(com.aotain.nms.common.model.smmscmd.LogQuery.CommandInfo info) {
		StringBuilder sb = new StringBuilder();
		int flag = 0;
		if(info.getProtocolType() != null) {
		...
		com.aotain.nms.common.model.smmscmd.LogQuery.CommandInfo.SrcIp tmp1 = info.getSrcIp();
		if (tmp1 != null) {
