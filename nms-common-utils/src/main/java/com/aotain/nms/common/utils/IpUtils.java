...
public class IpUtils {
	...
	/**
	 * 转成ipv6内嵌ipv4格式（IPV6段为标准格式）
	 * 
	 * @param bytes
	 * @param upperCase
	 * @return
	 */
	public static String parseIpv6StdInnerV4(byte[] bytes, boolean upperCase) {
		String ipv6Std = parseIpv6(bytes, upperCase).replaceAll("(\\:[a-fA-f\\d]{1,4}){1,2}$", "");
		long ipv4LongValue = (bytes[bytes.length - 4] << 24 & 0xff000000L) | (bytes[bytes.length - 3] << 16 & 0x00ff0000L)
				| (bytes[bytes.length - 2] << 8 & 0x0000ff00L) | (bytes[bytes.length - 1] & 0xff0000ffL);
		String ipv4Value = parseIpv4(ipv4LongValue);
		return ipv6Std + ":" + ipv4Value;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("10:100:200:0:0:0100:0000:0000 -> "+parseIpv6StdInnerV4(ipv6ToBytes("10:100:200:0:0:0100:0000:0000"), false));
		System.out.println("10:100:200:0:0:0100:ff0f:0f22 -> "+parseIpv6StdInnerV4(ipv6ToBytes("10:100:200:0:0:0100:ff0f:0f22"), false));
		System.out.println("2002:9D36:001:2:000:5EFE:10.10.10.10 -> "+parseIpv6StdInnerV4(ipv6ToBytes("2002:9D36:1:2:0:5EFE:10.10.10.10"), false));
	}
	...
	public static BigInteger ip2BigInteger(String ip) {
		...
		if (isIpv4(ip)) {
			result = new BigInteger(ipv4ToLong(ip) + "");
		} else if (isIpv6(ip)) {
			result = ipv6ToBigInteger(ip);
		}
		return result;
	}
	
	public static String ipToString(String ip) {
		return ip2BigInteger(ip).toString();
	}
