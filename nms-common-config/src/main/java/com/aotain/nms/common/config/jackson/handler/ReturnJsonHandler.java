package com.aotain.nms.common.config.jackson.handler;

import com.aotain.nms.common.config.annotation.JSON;
import com.aotain.nms.common.config.annotation.JSONS;
import com.aotain.nms.common.config.jackson.serialize.CustomerJsonSerializer;
...
public class ReturnJsonHandler implements HandlerMethodReturnValueHandler, BeanPostProcessor {
    ...
    public boolean supportsReturnType(MethodParameter returnType) {
        boolean hasJSONAnnotation = null != returnType.getMethodAnnotation(JSON.class)
                || null != returnType.getMethodAnnotation(JSONS.class);
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (bean instanceof ResponseBodyAdvice) {
            ...
            ReturnJsonHandler jsonHandler = null;
            for (int i = 0; i < handlers.size(); i++) {
                HandlerMethodReturnValueHandler handler = handlers.get(i);
                if (handler instanceof ReturnJsonHandler) {
                    jsonHandler = (ReturnJsonHandler) handler;
                    break;
                }
            }
            if (jsonHandler != null) {
